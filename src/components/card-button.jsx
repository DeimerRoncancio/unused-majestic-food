  ...
  return (
    ...
    onClick={onClick}>
      <div className='w-full h-full bg-center bg-cover rounded-2xl' style={{ backgroundImage: `url(${url})` }}>
        <div className='w-full h-full p-4 rounded-2xl bg-gradient-to-b from-transparent to-[#131313d0] flex flex-col
        items-start justify-end'>
          <h2 className='text-[#5bcc60] text-md font-medium'>{primaryText}</h2>
          <p className='text-xs text-white text-start font-extralight'>{secondaryText}</p>
        </div>
      </div>
    </button>
  )
