...
export default function DinamicInputText ({
  show, ifShow, loading, value, clickEdit, submit, hidde, inputName,
  inputValue, putValues, clickDelete
}) {
  ...
  return (
    <>
      ...
      <form onSubmit={submit} className={`${show ? '' : 'hidden'} flex ${ifShow ? 'justify-between' : ''}`}
        ...
        <button type='submit' className='text-[20px] text-green-600 hover:text-green-500'>
          <FaCheckCircle />
        </button>
      </form>
    </>
